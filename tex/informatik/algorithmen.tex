...
\paragraph{Heap}
\index{Heapsort}
Stelle Max-Heap (grö\ss tes Element in der Wurzel) her, gib Wurzel aus under ersetze mit Element ganz rechts in unterster Ebene.
...
(Die antisymmetrische Relation $\boldsymbol{\leq}$ hei\ss t Lexikographische Ordnung)
...
\begin{description}
  ...
  \item [Replacement Selectionsort] Lese $r < n$ Elemente auf Priority-Queue $Q$. Falls $x = \min(Q) \geq$ letztem Element auf zweiten Band, schreibe $x$ aus, sonst schreibe $Q$ auf Band. Wiederhole auf dritten Band und dann merge.
\end{description}
